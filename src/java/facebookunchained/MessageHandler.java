...
public class MessageHandler extends ReceiverAdapter implements MessageListener, ExceptionListener {
    ...
    public MessageHandler(String nombreDirectorio) {
        try {
    ...
    public MessageHandler(Directorio miDirectorio, String nombreDirectorio) {
    ...
    public static void main(String[] args) {
        try {
            String nd = "USA";
            Directorio d = new Directorio(nd);

            for (int i = 11; i <= 20; i++) {
                String nombre = String.valueOf(i);
                User u = new User(nombre, "User" + nombre, nd);
                d.agregaUsuario(u);
            }
            MessageHandler mh = new MessageHandler(d, nd);
            mh.start();
        } catch (Exception ex) {
            Logger.getLogger(MessageHandler.class.getName()).log(Level.SEVERE, null, ex);
        }
